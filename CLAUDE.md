...
|-------|-----------------|-------------|----------------|
| **Micro** | Single interaction | Request → response → feedback | Basic `_learn()` in integrations |
| **Meso** | Full agent run | Multiple steps in one execution | `_learn_with_trace()` (e.g., AgentExecutor) |
| **Macro** | Cross-run analysis | Comparing multiple runs | Future enhancement |

- **Micro-level insights**: Generated from single request/response cycles. Fast but limited context.
- **Meso-level insights**: Generated from full agent runs with intermediate steps (thoughts, actions, observations). Richer learning from agent reasoning traces.
